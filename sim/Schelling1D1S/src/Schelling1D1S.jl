...
export random_state, move!, gain, glauber_prob, asymmetric_utility, quadratic_utility, run_simulation!, run_simulation, save, run_sweep!, pick_sites
...
function run_simulation(state::Vector{Int64},
    ...
    # make sure we have a clean place to save output
    if isdir(savepath)
...
function pick_sites(state::Vector{Int64},
    ...
    from = sample(Weights(state))
    
    if moveType == "random"
        # pick a random place
        vacancies = capacity .- state
        to = sample(Weights(vacancies))
    ...
    elseif moveType == "local"
        neighbors = [mod1(from + 1, N),  # right
                     mod1(from - 1, N)]  # left
        vacancies = [capacity - state[n] for n in neighbors]
        
        # if all neighbors are full, pick a new "from" location
        while sum(vacancies) == 0
            from = sample(Weights(state))
            neighbors = [mod1(from + 1, N),  # right
                         mod1(from - 1, N)]  # left
            vacancies = [capacity - state[n] for n in neighbors]
        end
        new_site = sample(Weights(vacancies))
        to = neighbors[new_site]
    else
...
"""
function gain(state::Vector{Int64},
              from::T,
              to::T,
    ...
    from_old = local_average(state, from, kernel_size) / capacity
    to_old = local_average(state, to, kernel_size) / capacity
    from_new = local_average(new_state, from, kernel_size) / capacity
    to_new = local_average(new_state, to, kernel_size) / capacity
...
function local_average(arr::Vector{T}, site::T, box_size::T;
                       width::Float64 = 0.5) where T<:Integer
    kernel = Windows.gaussian(box_size, width) # create kernel
    kernel /= sum(kernel)  # normalize kernel
    half_box = Int(floor(box_size / 2))
    
    if site < half_box+1 || site > length(arr) - (half_box + 1)
        arr_periodic = Vector{Int64}(undef, length(arr) + (box_size - 1))
        arr_periodic = cat(arr[end-half_box+1:end], arr, arr[1:half_box], dims=1)
        psite = half_box + site
        c = sum(arr_periodic[psite - half_box:psite + half_box] .* kernel)
    else
        c = sum(arr[site - half_box:site + half_box] .* kernel)
    end
    return c
end
