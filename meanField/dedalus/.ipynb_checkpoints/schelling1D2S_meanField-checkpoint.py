...
if __name__ == "__main__":
    ...
    ### Schelling ####
    # add equation
    problem.add_equation("dt(ϕA) - T*dx(dx(ϕA)) = -dx(μA * ((1 - α) * dx(πA) + α * dx(dU_dϕA))) - dx(ΓA * μA * dx(dx(dx(ϕA))))")
    problem.add_equation("dt(ϕB) - T*dx(dx(ϕB)) = -dx(μB * ((1 - α) * dx(πB) + α * dx(dU_dϕB))) - dx(ΓB * μB * dx(dx(dx(ϕB))))")
    #################
