...
class CensusDataset(torch.utils.data.Dataset):
    ...
    def init_data(self):
        ...
        b_grid = []
        t = []
        with h5py.File(os.path.join(f'{self.data_dir}', f'{self.county}.hdf5'), 'r') as h5f:
            for year in h5f.keys():
                ...
                w_grid.append(np.ma.array(gaussian_filter(g['white_grid'][:], sigma=self.sigma),
                                          mask=area_mask, fill_value=np.nan).filled())
                b_grid.append(np.ma.array(gaussian_filter(g['black_grid'][:], sigma=self.sigma),
                                          mask=area_mask, fill_value=np.nan).filled())
